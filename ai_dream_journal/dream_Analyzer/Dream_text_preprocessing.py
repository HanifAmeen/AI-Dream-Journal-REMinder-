...
def remove_itemized_bullet_and_numbering(input_text: str) -> str:
    ...
    processed_text = re.sub(r'[(\s][0-9a-zA-Z][.)]\s+|[(\s][ivxIVX]+[.)]\s+', ' ', input_text)
    return processed_text
...
def preprocess_dream_narrative(input_text: str) -> str:
    """
    Light preprocessing for dream interpretation.
    Preserves narrative, emotion, and agency.
    Keeps: pronouns, tense, punctuation (light), emotional emphasis, names
    """
    processing_function_list = [
        normalize_unicode,              # fix encoding only
        remove_url,
        remove_email,
        remove_phone_number,
        ...
        expand_contraction,
        remove_whitespace,              # normalize spacing
    ]
...
def preprocess_dream_semantic(input_text: str) -> str:
    """
    Strong preprocessing for semantic models only.
    Used for embeddings, symbol search, topic modeling.
    Reduces vocabulary noise, stabilizes embeddings, improves similarity search.
    """
    processing_function_list = [
        to_lower,
        normalize_unicode,
        ...
        remove_punctuation,
        remove_whitespace,
        lemmatize_word,                 # semantic normalization
    ]
...
def preprocess_text(input_text: str, processing_function_list: Optional[List[Callable]] = None) -> str:
    """
    Legacy preprocessing function - use dream-specific pipelines instead.
    WARNING: May destroy dream narrative structure.
    """
...
def process_dreambank_csv_row(raw_text: str) -> dict:
    """
    Process a single dreambank row into narrative and semantic versions.
    Returns: {'narrative_text': str, 'semantic_text': str}
    """
    narrative_text = preprocess_dream_narrative(raw_text)
    semantic_text = preprocess_dream_semantic(raw_text)
    return {
...
if __name__ == '__main__':
    # Test the new dream pipelines
    dream_text = 'I was running!!! My mother kept calling my name, but the room felt WRONG. john.doe@email.com www.test.com'
    
    print("=== RAW ===")
    print(dream_text)
    print()
    
    print("=== NARRATIVE (Interpretation-Safe) ===")
    narrative = preprocess_dream_narrative(dream_text)
    print(narrative)
    print()
    
    print("=== SEMANTIC (Embeddings-Safe) ===")
    semantic = preprocess_dream_semantic(dream_text)
    print(semantic)
    print()
    
    print("=== DreamBank Row Processing ===")
    result = process_dreambank_csv_row(dream_text)
    print("Narrative:", result['narrative_text'])
    print("Semantic:", result['semantic_text'])
